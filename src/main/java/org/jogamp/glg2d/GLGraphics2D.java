...
{
	/**
	 * The parent graphics object, if we have one. This reference is used to
	...
	public void removeG2DDrawingHelper(G2DDrawingHelper helper)
	{
		for (int i = 0; i < helpers.length; i++)
		{
			if (helpers[i] == helper)
			{
				System.arraycopy(helpers, i + 1, helpers, i, helpers.length
				        - (i + 1));
				helpers = Arrays.copyOf(helpers, helpers.length - 1);
				break;
			}
	...
	/**
	 * Sets up the graphics object in preparation for drawing. Initialization
	 * such as getting the viewport
	 */
	...
	/**
	 * Provides access to a particular GL implementation.
	 * 
	 * @param glType
	 *            - the type of GL profile we wish to use.
	 * @return the profile for performing GL operations.
	 */
	public <T extends GL> T getGL(Class<T> glType)
	{
		GL gl = glContext.getGL();

		if (GL.class.equals(glType))
		{
			return glType.cast(gl.getGL());
		}
		else if (GL2.class.equals(glType))
		{
			return glType.cast(gl.getGL2());
		}
		else if (GL2ES2.class.equals(glType))
		{
			return glType.cast(gl.getGL2ES2());
		}

		throw new IllegalArgumentException("Unsupported GL Profile: " + glType);
	}
	...
	public void setRenderingHint(Key hintKey, Object hintValue)
	{
		...
		{
			throw new IllegalArgumentException(hintValue
			        + " is not compatible with " + hintKey);
		}
		else
		{
			for (G2DDrawingHelper helper : helpers)
	...
	public Rectangle getClipBounds()
	{
		...
		else
		{
			try
			{
				...
				int minX = (int) Math.min(pts[0],
				        Math.min(pts[2], Math.min(pts[4], pts[6])));
				int maxX = (int) Math.max(pts[0],
				        Math.max(pts[2], Math.max(pts[4], pts[6])));
				int minY = (int) Math.min(pts[1],
				        Math.min(pts[3], Math.min(pts[5], pts[7])));
				int maxY = (int) Math.max(pts[1],
				        Math.max(pts[3], Math.max(pts[5], pts[7])));
				return new Rectangle(minX, minY, maxX - minX, maxY - minY);
			}
			catch (NoninvertibleTransformException e)
			{
				// Not sure why this would happen
				Logger.getLogger(GLGraphics2D.class.getName()).log(
				        Level.WARNING, "User transform is non-invertible", e);
	...
	protected void setClip(Rectangle2D clipShape, boolean intersect)
	{
		...
		{
			Rectangle rect = getTransform().createTransformedShape(clipShape)
			        .getBounds();
	...
	{
		GL gl = getGL(GL.class);
		if (enable)
		{
			gl.glScissor(clip.x, canvasHeight - clip.y - clip.height,
			        Math.max(clip.width, 0), Math.max(clip.height, 0));
	...
	{
		shapeHelper.drawRoundRect(x, y, width, height, arcWidth, arcHeight,
		        false);
	}
	...
	{
		return imageHelper.drawImage(img, x, y, width, height, bgcolor,
		        observer);
	}
	...
	{
		return imageHelper.drawImage(img, dx1, dy1, dx2, dy2, sx1, sy1, sx2,
		        sy2, null, observer);
	}
	...
	{
		/*
		 * This is also called on the finalizer thread, which should not make
		 * OpenGL calls. We also want to make sure that this only executes once.
		 */
		if (!isDisposed)
		{
			isDisposed = true;
