...
public class MouseEventTranslator
{
	...
	/**
	 * Sets the {@code AffineTransform} that is used to transform a point
	 * relative to the origin component into a point relative to the target
	 * component.
	 */
	...
	/**
	 * Gets the {@code AffineTransform} that is used to transform a point
	 * relative to the origin component into a point relative to the target
	 * component.
	 * 
	 * <p>
	...
	public MouseEvent publishMouseEvent(Component source, int id, long when,
	        int modifiers, Point sourcePoint, int clickCount, int button)
	{
		// TODO how to determine this?
		boolean isPopupTrigger = false;
		MouseEvent e = new MouseEvent(source, id, when, modifiers,
		        sourcePoint.x, sourcePoint.y, 0, 0, clickCount, isPopupTrigger,
		        button);
		publish(e);
		return e;
	}

	public MouseEvent publishMouseEvent(int id, long when, int modifiers,
	        int clickCount, int button, Point clickedOnOrigin)
	{
		...
		return publishMouseEvent(source, id, when, modifiers, clickedOnSource,
		        clickCount, button);
	}

	public MouseWheelEvent publishMouseWheelEvent(Component source, int id,
	        long when, int modifiers, Point sourcePoint, int clickCount,
	        int scrollType, int scrollAmount, int wheelRotation)
	{
		// TODO how to determine this?
		boolean isPopupTrigger = false;
		MouseWheelEvent e = new MouseWheelEvent(source, id, when, modifiers,
		        sourcePoint.x, sourcePoint.y, 0, 0, clickCount, isPopupTrigger,
	...
	public MouseWheelEvent publishMouseWheelEvent(int id, long when,
	        int modifiers, int wheelRotation, Point mouseOnOrigin)
	{
		...
		return publishMouseWheelEvent(source, id, when, modifiers,
		        clickedOnSource, 0, MouseWheelEvent.WHEEL_UNIT_SCROLL,
		        abs(wheelRotation), wheelRotation);
	}

	/**
	 * Converts a point that is relative to the origin component into a point
	 * that is relative to the target component.
	 */
	 ...
	 */
	protected Point targetPointToSource(final Component source, final Point pt)
	{
	 ...
	 * Gets the source component at the given target-relative point.
	 */
	protected Component getSourceComponent(int x, int y)
	{
		return SwingUtilities.getDeepestComponentAt(target, x, y);
	}
