...
public abstract class AbstractImageHelper implements GLG2DImageHelper
{
	private static final Logger LOGGER = Logger
	        .getLogger(AbstractImageHelper.class.getName());
	...
	protected abstract void begin(Texture texture, AffineTransform xform,
	        Color bgcolor);

	protected abstract void applyTexture(Texture texture, int dx1, int dy1,
	        int dx2, int dy2, float sx1, float sy1, float sx2, float sy2);
	        ...
	        ImageObserver observer)
	{
		...
		applyTexture(texture, dx1, dy1, dx2, dy2, sx1 / width, sy1 / height,
		        sx2 / width, sy2 / height);
		end(texture);
	...
	protected void applyTexture(Texture texture)
	{
		...
		applyTexture(texture, 0, 0, width, height, coords.left(), coords.top(),
	 ...
	 * suspect I should be using the ImageConsumer class and dumping pixels to
	 * the screen as I receive them.
	 * 
	 * <p>
	 ...
	 * available. If it is, cache it. If it's not, don't cache it. But if not
	 * all the image data is available, we will draw it what we have, since we
	 * draw anything in the image to a BufferedImage.
	 * </p>
	 */
	...
	protected void addToCache(Image image, Texture texture)
	{
		if (clearCachePolicy instanceof Number)
		{
			...
			if (imageCache.size() > maxSize)
			{
				...
				{
					LOGGER.fine("Clearing texture cache with size "
					        + imageCache.size());
				}
